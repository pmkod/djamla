...
const basePaginationButtonClassName =
  "size-9 aspect-square flex items-center justify-center rounded-sm hover:bg-gray-100 data-[disabled]:hover:bg-transparent transition-colors";
          ...
          pagination.pages.map((page, index) =>
            page.type === "page" ? (
              <PaginationPrimitives.Item
                key={index}
                className={`${basePaginationButtonClassName} border border-gray-300 data-[selected]:border-indigo-600 data-[selected]:bg-indigo-600 data-[selected]:text-white`}
                {...page}
              >
                {page.value}
              </PaginationPrimitives.Item>
